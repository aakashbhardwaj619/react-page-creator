...
export class SPService {
   ...
   */
	 ...
	 */
	public static async GETALLSITES(msGraphClientFactory: MSGraphClientFactory): Promise<IPropertyPaneDropdownOption[]> {
		let selectedSites: IPropertyPaneDropdownOption[] = [];
		
		let _msGraphClient: MSGraphClient = await msGraphClientFactory.getClient();
		let response = await _msGraphClient.api(`/sites?search=*&$select=displayName,webUrl`).get();
